...
def train(opt):
    ...
    model = UNet(img_channels=opt.img_channels, is_cond_image=True, channel_mults=(1, 2, 4, 4), attention_layers=(0, 1))
    ...
    for epoch in range(trained_epoch + 1, trained_epoch + n_epochs + 1):
        ...
        for batch_idx, imgs in tqdm(enumerate(train_dataloader), desc=f'Training Epoch {epoch}',
            ...
            loss = diffusion(x=images_optical, cond_image=images_sar)
            loss.backward()
            optimizer.step()
        ...
        if epoch % opt.save_epoch_rate == 0 or (epoch == (trained_epoch + n_epochs)):
            diffusion.eval()
            samples = diffusion.sample(batch_size=opt.batch_size, device=device, cond_image=show_image_sar).to(device)
